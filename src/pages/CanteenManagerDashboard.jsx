...
import { Download, Printer, Plus, Edit, Trash2, LogOut, Utensils, CheckCircle2, XCircle, Power, PowerOff, Star, MessageSquare, PieChart, ChefHat, RefreshCw } from 'lucide-react'; 
...
const CanteenManagerDashboard = () => {
  ...
  const [isRefreshing, setIsRefreshing] = useState(false); // 🚀 NEW: Loading state for auto-refresh
  ...
  // 🚀 FIX: AUTO-REFRESH LOGIC (Polling every 10 seconds)
  useEffect(() => {
    // Initial fetch
    fetchOrders();
    fetchDepartments();
    ...
    // Set up the interval for automatic refresh
...
const downloadReport = () => {
      ...
      img.onload = () => {
        ...
        doc.setFont("helvetica", "bold");
        const deptTitle = filterDept === 'All Departments' ? "ALL DEPARTMENTS" : `${filterDept.toUpperCase()}`;
        ...
        doc.setFontSize(9); doc.setFont("helvetica", "bold"); doc.line(20, signatureY, 60, signatureY); doc.text("MESS MANAGER", 25, signatureY + 6);
        ...
        doc.setFont("helvetica", "normal"); doc.setFontSize(7); const downloadTime = new Date().toLocaleString('en-GB');
        doc.text(`SYSTEM GENERATED REPORT | PICT CANTEEN & MESS SECTION | DOWNLOADED: ${downloadTime}`, 48, pageHeight - 5);
                  ...
                  <div className="animate-in fade-in duration-300">
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                         <div className="bg-slate-50 p-5 rounded-2xl border flex flex-col gap-3 shadow-inner">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                               ...
                               {Object.keys(categoryRevenue).length === 0 && <div className="col-span-2 py-4 text-center text-xs text-slate-400 italic">No sales data</div>}
                            </div>
                         </div>
                         <div className="bg-slate-50 p-5 rounded-2xl border flex flex-col gap-3 shadow-inner col-span-1 lg:col-span-2">
                            ...
                            <div className="flex flex-wrap gap-3">
                                ...
                                {popularItems.length === 0 && <p className="text-slate-400 text-xs italic">No orders in range.</p>}
                            </div>
                         </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-end gap-5 mb-8 bg-slate-50 p-5 rounded-2xl border border-slate-200 shadow-inner">
                          <div className="flex-1">
                              ...
                              <select value={filterDept} onChange={(e) => setFilterDept(e.target.value)} className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm font-semibold outline-none focus:border-blue-500 bg-white shadow-sm transition-all">
                              ...
                              <tbody className="divide-y-2 divide-slate-50 text-sm">
                                  {filteredOrders.length === 0 ? (
                                      ...
                                      filteredOrders.map(order => (
                                          <tr key={order._id} className="hover:bg-blue-50/50 transition-colors bg-white group animate-in fade-in slide-in-from-top-1 duration-300">
                                              ...
                                              <td className="py-4 px-6 text-slate-500 text-xs font-bold">{order.createdAt || order.orderDate ? new Date(order.createdAt || order.orderDate).toLocaleString('en-GB', {day: '2-digit', month: 'short', hour: '2-digit', minute:'2-digit'}) : 'No Date'}</td>
              ...
              {activeTab === 'menu' && (
                  <div className="animate-in fade-in duration-300">
                      <div className="flex justify-between items-center mb-8 px-2">
                          ...
                          <button onClick={() => { setMenuForm({itemName:'', category:'Snacks', price:''}); setEditingItemId(null); setIsMenuModalOpen(true); }} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-blue-700 shadow-lg active:scale-95 transition-all"><Plus size={18} /> Add New Item</button>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                          {menuItems.map(item => (
                              <div key={item._id} className={`border-2 p-5 rounded-2xl flex justify-between items-center transition-all bg-white relative overflow-hidden group ${item.isAvailable !== false ? 'border-slate-100 hover:border-blue-300 hover:shadow-md' : 'border-red-100 bg-red-50/30 opacity-75'}`}>
                                  <div>
                  ...
                  <form onSubmit={handleMenuSubmit} className="space-y-5">
                      ...
                      <div>
                          <div className="flex justify-between items-end mb-2">
                              ...
                              <span className="text-[10px] font-bold text-blue-500 bg-blue-50 px-2 py-0.5 rounded">Max: ₹{categoryPriceLimits[menuForm.category] || 'N/A'}</span>
                          </div>
