...
const CanteenManagerDashboard = () => {
  ...
  const cancelOrder = async (id) => {
    if(window.confirm("Permanently delete this order from database?")) {
        try {
  ...
  const downloadReport = () => {
    ...
    img.onload = () => {
      ...
      doc.rect(55, 30, 100, 8); doc.text("DEPARTMENT-WISE BILLING REPORT", 62, 36);
      doc.setFontSize(10);
      ...
      facultyOrders.forEach(order => {
          ...
          if (!facultyTotals[key]) facultyTotals[key] = { name: baseName, date: rawDate, details: year, items: [], total: 0 };
      ...
      guestOrders.forEach(order => {
          ...
          if (!guestTotals[key]) guestTotals[key] = { name: actualGuestName, date: rawDate, details: "Guest Voucher", items: [], total: 0 };
      ...
      autoTable(doc, { startY: currentY + 3, head: [['Sr', 'Date', 'Name', 'Year', 'Items', 'Total']], body: fTable, theme: 'grid', bodyStyles: { fontStyle: 'bold', fontSize: 9.5 } });
      ...
      autoTable(doc, { startY: currentY + 3, head: [['Sr', 'Date', 'Name', 'Type', 'Items', 'Total']], body: gTable, theme: 'grid', bodyStyles: { fontStyle: 'bold', fontSize: 9.5 } });
    ...
    <div className="min-h-screen bg-[#f8f9fc] font-sans text-slate-800">
      <header className="bg-white border-b sticky top-0 z-20 shadow-sm flex justify-between items-center px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-black uppercase border ${isRefreshing ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-slate-50 text-slate-400'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-sm border p-6 min-h-[600px]">
                      ...
                      <div className="flex flex-wrap items-end gap-4 mb-8 bg-slate-50 p-5 rounded-2xl border shadow-inner">
                          <div className="flex-1 min-w-[250px] relative">
                             ...
                             <div className="relative">
                                ...
                                <input type="text" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 p-2.5 border-2 border-slate-200 rounded-xl text-sm font-semibold outline-none focus:border-blue-500" />
                             </div>
                          </div>
                          <div><label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Department</label>
                          <select value={filterDept} onChange={(e) => setFilterDept(e.target.value)} className="p-2.5 border-2 border-slate-200 rounded-xl text-sm font-semibold bg-white outline-none focus:border-blue-500">
                          ...
                          <table className="w-full text-left border-collapse">
                              ...
                              <tbody className="divide-y-2 divide-slate-50 text-sm font-medium">
                                  {filteredOrders.map(order => (
                                          <tr key={order._id} className="hover:bg-blue-50/50 bg-white">
              ...
              {activeTab === 'menu' && (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    <div className="col-span-full flex justify-between mb-4"><h2 className="text-xl font-black">Menu Items</h2><button onClick={() => { setMenuForm({itemName:'', category:'Snacks', price:''}); setEditingItemId(null); setIsMenuModalOpen(true); }} className="bg-blue-600 text-white px-5 py-2 rounded-xl font-bold flex items-center gap-2"><Plus size={18} /> Add Item</button></div>
                    {menuItems.map(item => (
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
              <div className="bg-white p-8 rounded-3xl w-full max-w-sm shadow-2xl animate-in zoom-in duration-200 border border-slate-100">
                  ...
                  <form onSubmit={handleMenuSubmit} className="space-y-4">
                      ...
                      <div className="flex gap-3 pt-4"><button type="button" onClick={() => setIsMenuModalOpen(false)} className="flex-1 py-3.5 border-2 font-bold rounded-xl">Cancel</button><button type="submit" className="flex-1 py-3.5 bg-blue-600 text-white font-black rounded-xl shadow-lg">Save</button></div>
                  </form>
              </div>
          </div>
      )}
