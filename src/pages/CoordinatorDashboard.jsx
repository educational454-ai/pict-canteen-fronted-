...
const CoordinatorDashboard = () => {
  ...
  const handleExportReportsCSV = () => {
    ...
    XLSX.writeFile(workbook, `${deptCode}_Usage_Report.xlsx`);
  ...
  const generatePDFInvoice = () => {
    ...
    img.onload = () => {
      ...
      // 🚀 PDF Visibility Fix: Bold + Size 9.5
      autoTable(doc, {
        startY: currentY + 3,
      ...
      doc.rect(130, currentY + 15, 66, 10); doc.setFontSize(13); doc.text(`GRAND TOTAL`, 135, currentY + 22); doc.text(`Rs. ${grandTotal}`, 175, currentY + 22);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to revoke this voucher?")) {
      try { await API.delete(`/faculty/remove/${id}`); toast.success("Voucher Revoked."); fetchFaculty(); } 
      catch (err) { toast.error("Failed to delete."); } 
    }
  };

  const handleResetSystem = async () => {
     if(window.confirm(`WARNING: This will deactivate ALL faculty records for ${deptCode}.`)) {
         try { await API.delete(`/faculty/department/${deptId}/reset`); setFaculty([]); toast.success(`System Reset.`); } 
  ...
  const handleAddSingle = async (e) => {
    e.preventDefault();
    if (!deptId || !deptCode) { toast.error("Re-login required."); return; }
    try {
  ...
  const handleFileUpload = (e) => { 
    ...
    reader.onload = async (evt) => {
      ...
      try { await API.post('/faculty/bulk-add', Array.from(facultyMap.values())); toast.success("Upload Successful!"); fetchFaculty(); } catch (err) { toast.error("Format Error."); }
    };
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-[#f8f9fc] relative h-screen overflow-hidden">
      <div className="w-64 bg-[#0a1128] text-white flex flex-col shadow-2xl z-10 hidden md:flex">
        ...
        <nav className="flex-1 p-4 space-y-2 mt-2">
          <button onClick={() => setActiveTab('faculty')} className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold transition-all ${activeTab === 'faculty' ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400 hover:text-white'}`}><Users size={20} /> Faculty Management</button>
          <button onClick={() => setActiveTab('guests')} className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold transition-all ${activeTab === 'guests' ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400 hover:text-white'}`}><Ticket size={20} /> Guest Vouchers</button>
          <button onClick={() => setActiveTab('reports')} className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold transition-all ${activeTab === 'reports' ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400 hover:text-white'}`}><BarChart3 size={20} /> Reports & Logs</button>
        </nav>
        ...
        {activeTab === 'faculty' && (
          <main className="flex-1 flex flex-col p-4 md:p-8 gap-4 overflow-hidden">
            ...
            <div className="bg-white p-4 rounded-2xl border shadow-sm flex gap-4 shrink-0 items-center">
              <div className="relative flex-1"><Search className="absolute left-4 top-3.5 text-gray-400" size={18} /><input type="text" placeholder="Search..." className="w-full pl-12 pr-4 py-2.5 border-2 border-gray-100 rounded-xl text-sm outline-none focus:border-blue-500" onChange={(e) => setSearchTerm(e.target.value)} /></div>
              <select className="px-4 py-2.5 border-2 border-gray-100 rounded-xl text-gray-600 text-sm font-bold bg-white outline-none" value={yearScope} onChange={(e) => setYearScope(e.target.value)}>
                  <option value="">All Years</option>{availableYears.map(y => <option key={y} value={y}>{y}</option>)}
              </select>
              <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
              <button onClick={() => fileInputRef.current.click()} className="px-4 py-2.5 border-2 border-yellow-100 rounded-xl text-sm font-bold text-yellow-700 bg-yellow-50 flex items-center gap-2 transition-all hover:bg-yellow-100"><FileSpreadsheet size={16} /> Upload</button>
              <button className="px-4 py-2.5 bg-blue-600 text-white rounded-xl font-bold text-sm shadow-lg flex items-center gap-2 transition-all hover:bg-blue-700 active:scale-95" onClick={() => setIsModalOpen(true)}><Plus size={18} /> New</button>
            </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y text-sm">
                  {filteredFaculty.map((f) => {
                    const isToday = new Date().toDateString() === new Date(f.validTill).toDateString();
                    return (
                    <tr key={f._id} className={`${isToday ? 'bg-amber-50/50' : 'hover:bg-gray-50/50'}`}>
        ...
        <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm flex justify-center items-center z-50 px-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleAddSingle} className="space-y-4">
              <input required type="text" placeholder="Full Name" value={formData.fullName} onChange={(e) => setFormData({...formData, fullName: e.target.value})} className="w-full p-3 border rounded-lg outline-none focus:border-blue-500" />
              <input required type="email" placeholder="Email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full p-3 border rounded-lg outline-none focus:border-blue-500" />
              <input required type="text" placeholder="Mobile" value={formData.mobile} onChange={(e) => setFormData({...formData, mobile: e.target.value})} className="w-full p-3 border rounded-lg outline-none focus:border-blue-500" />
              <input type="text" placeholder="Subject Name" value={formData.assignedSubject} onChange={(e) => setFormData({...formData, assignedSubject: e.target.value})} className="w-full p-3 border rounded-lg outline-none focus:border-blue-500" />
              <div><label className="text-[11px] font-bold text-gray-400 uppercase">Year Scope</label><select value={formData.academicYear} onChange={(e) => setFormData({...formData, academicYear: e.target.value})} className="w-full p-3 border rounded-lg bg-white outline-none focus:border-blue-500">{availableYears.map(y => <option key={y} value={y}>{y}</option>)}</select></div>
              <div className="flex gap-4">
                <div className="flex-1"><label className="text-[10px] font-bold text-gray-400">From</label><input type="date" value={formData.validFrom} onChange={(e) => setFormData({...formData, validFrom: e.target.value})} className="w-full p-2 border rounded-lg text-sm" /></div>
                <div className="flex-1"><label className="text-[10px] font-bold text-gray-400">Till</label><input type="date" value={formData.validTill} onChange={(e) => setFormData({...formData, validTill: e.target.value})} className="w-full p-2 border rounded-lg text-sm" /></div>
              </div>
        ...
        <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm flex justify-center items-center z-50 px-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleAddGuest} className="space-y-4">
              <input required type="text" placeholder="Guest Name" value={guestFormData.guestName} onChange={(e) => setGuestFormData({...guestFormData, guestName: e.target.value})} className="w-full p-3 border rounded-lg outline-none focus:border-purple-500" />
              <input required type="text" placeholder="Host Voucher" value={guestFormData.facultyVoucher} onChange={(e) => setGuestFormData({...guestFormData, facultyVoucher: e.target.value})} className="w-full p-3 border rounded-lg outline-none focus:border-purple-500" />
              <button type="submit" className="w-full bg-purple-600 text-white font-bold p-3 rounded-lg mt-4 hover:bg-purple-700">Issue Code</button>
            </form>
          </div>
        </div>
      )}
