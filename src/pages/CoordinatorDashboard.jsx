...
const CoordinatorDashboard = () => {
  ...
  const buildVoucherMessage = (member) => {
    ...
    return `*PICT EXAM PORTAL — CANTEEN VOUCHER*\n\nDear Prof. *${member.fullName}*,\n\nYou have been assigned as an examiner for the *${deptCode}* Department.\n\n*ONE-TAP ACCESS (tap or long-press to copy):*\n${accessLink}\n\n*VOUCHER DETAILS*\n- Exam Category: ${examCategory}\n- Access Code: ${member.voucherCode}\n- Valid: ${new Date(member.validFrom).toLocaleDateString('en-GB')} — ${new Date(member.validTill).toLocaleDateString('en-GB')}\n\n*HOW TO USE*\n1) Tap the link above to open the portal (the link contains your code).\n2) Long-press the link in WhatsApp to copy the URL (the code is included).\n3) Open the app and press *Login*, then paste the access code when prompted.\n\n*NOTES*\n- The voucher is personal and valid for one order per day.\n- If you face issues, contact your Department Coordinator.\n\nKind Regards,\nDepartment Coordinator — PICT`; 
  };
